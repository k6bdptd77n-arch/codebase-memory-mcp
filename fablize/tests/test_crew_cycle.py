...
class CycleTests(unittest.TestCase):
    ...
    def test_complete_verdict_checkpoints_then_merges(self):
        self.mc.review = lambda sid: "VERDICT: COMPLETE — evidence checks out"
        self.mc.cycle()
        self.assertEqual(self._ops(), ["run", "next", "verify", "checkpoint", "merge", "reflect"])
        self.assertEqual(self.calls[3][1:3], ("G001", "complete"))

    def test_failed_verdict_never_merges(self):
        self.mc.review = lambda sid: "VERDICT: FAILED — no tests in the log"
        self.mc.cycle()
        self.assertEqual(self._ops(), ["run", "next", "checkpoint", "reflect"])
        self.assertEqual(self.calls[2][1:3], ("G001", "failed"))
    ...
    def test_skips_story_goals_next_did_not_activate(self):
        ...
        self.mc.review = lambda sid: "VERDICT: COMPLETE — looks fine"
        self.mc.cycle()
        self.assertEqual(self._ops(), ["run", "next", "reflect"])

    def test_verify_cmd_env_override_threads_to_gate_and_checkpoint(self):
        self.mc.review = lambda sid: "VERDICT: COMPLETE — ok"
